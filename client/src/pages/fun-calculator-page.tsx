...
export default function FunCalculatorPage() {
  ...
  useEffect(() => {
    if (mealType) {
      ...
      if (mealType === "longActing" && settings?.longActingDosage) {
        ...
        setShouldUseTypewriter(true); // Use typewriter effect
      ...
      // For all other meal types, proceed normally
        ...
        setDisplayValue("0");
        setShouldUseTypewriter(false); // Regular display initially
      }
    }
  }, [mealType, settings]);
  ...
  useEffect(() => {
    if (bgValue !== null && wizardStep === 'bg') {
      ...
      if (mealType === 'bedtime') {
        ...
        // Auto clear display for number entry
        setDisplayValue("0");
        setShouldUseTypewriter(false); // Regular display for numbers
  ...
  const inputDigit = (digit: string) => {
    ...
    if (waitingForSecondOperand) {
      ...
      setWaitingForSecondOperand(false);
      setShouldUseTypewriter(true); // Activate typewriter effect for new input
    } else {
      // Apply regular input, typewriter effect is managed separately
  ...
  const inputDecimal = () => {
    ...
    if (waitingForSecondOperand) {
      setDisplayValue("0.");
      setWaitingForSecondOperand(false);
      setShouldUseTypewriter(true); // Activate typewriter effect
      return;
    }
  ...
  const handleOperator = (nextOperator: string) => {
    ...
    if (previousValue === null) {
      ...
      setShouldUseTypewriter(true); // Activate typewriter effect for result
  ...
  const clearEntry = () => {
    ...
    setWaitingForSecondOperand(false);
    setCalculationHistory([]);
    setShouldUseTypewriter(false); // Disable typewriter for clear
  };
  ...
  const toggleCarbTotalMode = () => {
    ...
    if (carbTotalMode) {
      ...
      setOperation(null);
      setWaitingForSecondOperand(false);
      setShouldUseTypewriter(false); // Disable typewriter initially
      setCalculationHistory([]);
    }
  };

  // Handle voice input results (disabled, but keeping function for future use)
  const handleVoiceResult = (transcript: string) => {
    // This function is kept for compatibility but will not be used
    console.log("Voice input disabled, received transcript:", transcript);
  };
  ...
  const selectFoodPortion = (carbValue: number) => {
    ...
    setShouldUseTypewriter(true); // Use typewriter for selected food
  ...
  // Voice input start notification - disabled but keeping for future use
  const notifyVoiceInputStarted = () => {
    // This function is kept for compatibility but does nothing now
  };

  // Render the insulin calculator with enhanced typewriter display
  return (
        ...
        <Card className="bg-gray-900 border-gray-800 overflow-hidden">
          ...
          <div className="p-3 bg-gray-800">
            ...
            <div className="flex flex-col gap-2">
              ...
              {/* Current BG and Carb Total - Second row - Combined buttons without voice input */}
